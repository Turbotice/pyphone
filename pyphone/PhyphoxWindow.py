...
class PhyphoxWindow(SecondaryWindow):
    def __init__(self,mainWindow):
        ...
        buttons_list[0] = { 'Start Phyphox':self.act_phyphox_start,\
                            ...
                            'Restart':self.act_phyphox_restart}                        
...
#        return connect.launch_phyphox(self.phone_toconnect)
        #r = asyncio.run(self.async_launch_phyphox())
        #iplist = connect.get_adresslist(self.phone_toconnect)
        #print(iplist)
#        await asyncio.gather(*map(connect.launch_phyphox,self.phone_toconnect))

        t1 = Process(target=self.launch_phyphox)
        t1.start()
...
#        connect.launch_phyphox(phone)   
        #coroutine = await asyncio.gather(*map(connect.launch_phyphox,self.phone_toconnect))#run_phyphox.execute(connect.launch_phyphox,iplist)
        #loop.run_until_complete(coroutine)
        #return coroutine

    def act_phyphox_restart(self):
        adrlist,self.phonelist = connect.connect()   
        self.pt('starting phyphox on '+str(self.phonelist))
        for phone in self.phonelist:
            connect.launch_phyphox(phone)            
        self.act_phyphox_check()
